...
func TestWithMethod(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			if tt.fn() == nil {
				got = NewOptions(defaultExpire, encryptionKey).Method
			} else {
				got = NewOptions(defaultExpire, encryptionKey, tt.fn()).Method
			}
			assert.Equal(t, tt.want, got)
		})
	}
...
func TestWithIssuer(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			var got string
			if tt.fn() == nil {
				got = NewOptions(defaultExpire, encryptionKey).Issuer
			} else {
				got = NewOptions(defaultExpire, encryptionKey, tt.fn()).Issuer
			}
			assert.Equal(t, tt.want, got)
		})
	}
...
func TestWithGenIDFunc(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			var got string
			if tt.fn() == nil {
				got = NewOptions(defaultExpire, encryptionKey).genIDFn()
			} else {
				got = NewOptions(defaultExpire, encryptionKey, tt.fn()).genIDFn()
			}
			assert.Equal(t, tt.want, got)
		})
	}
