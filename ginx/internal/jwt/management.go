...
type Management[T any] struct {
	...
	nowFunc            func() time.Time // 控制 jwt 的刷新时间
}
...
// WithAllowTokenHeader 设置 token 的请求头
...
func (m *Management[T]) Refresh(ctx *gin.Context) {
	...
	if err != nil {
		slog.Error("refresh token verification failed")
		ctx.Status(http.StatusUnauthorized)
		return
	}
	accessToken, err := m.GenerateAccessToken(clm.Date)
	if err != nil {
	...
	if m.rotateRefreshToken {
		refreshToken, err := m.GenerateRefreshToken(clm.Date)
		if err != nil {
...
func (m *Management[T]) VerifyAccessToken(token string, opts ...jwt.ParserOption) (RegisteredClaims[T], error) {
	t, err := jwt.ParseWithClaims(token, &RegisteredClaims[T]{},
		func(token *jwt.Token) (interface{}, error) {
			return []byte(m.accessJWTOptions.DecryptKey), errEmptyRefreshOpts
		},
		opts...,
	)
	if err != nil || !t.Valid {
